...
printf("Left Sensor: %f, Right Sensor: %f, Front Sensor: %f\n", wb_distance_sensor_get_value(prox_sensors[5]), wb_distance_sensor_get_value(prox_sensors[2]), wb_distance_sensor_get_value(prox_sensors[7]));
    ...
    if (right_wall && left_wall && front_wall && value_count < 30) {
      ...
      printf("\nStoring light and distance sensor values at count %d\n", value_count);
      value_count++;
    }
    else  {
    printf("\n I did not get into a corner\n");
